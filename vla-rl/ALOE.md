...
> arXiv:2602.12691 · [项目页](https://rooshy-yang.github.io/aloe/)  
> 相关对照：[`LWD.md`](./LWD.md)、[`pi0.6*.md`](./pi0.6*.md)、[`GR-RL.md`](./GR-RL.md)  
>
...
论文在真实机器人操纵任务上评估 ALOE（不同版本表述为 3–4 项），覆盖高精度与长时序场景，例如：

- **手机装壳 / 组装**：高精度接触任务；
- **折叠衣物**：长时序、柔性物体任务；
- **多物体分拣 / 双臂放置**：多物体感知与双臂协作。
...
作者特别强调，在手机装壳任务上，成功率提升并没有以更慢执行速度为代价；相反，throughput 更高，说明它既更准也更快。
...
这说明它学到的不只是粗糙的 trajectory score，而是真正能区分关键 action chunk 的价值函数。

## 14.3 ALOE 在多轮 RL 迭代中还能持续提升

论文比较了多轮迭代结果，指出某些保守 baseline 在前几轮提升后容易饱和，而 ALOE 在持续迭代中还能继续改进。  
作者把这一点归因于：**action-level off-policy value estimation 提供了 trajectory-level 方法没有的额外学习信号。**
...
# 17. 和 LWD / Recap 的快速对照

| | ALOE | LWD | Recap |
|--|------|-----|-------|
| 场景 | HITL 真实后训练，强调异策略碎片数据 | 机群部署飞轮，多任务通才 | 单任务大规模经验后训练 |
| 价值 | chunk TD + 悲观 ensemble | DIVL（分布 IQL 系） | distributional V + advantage |
| 策略 | advantage-weighted flow | QAM（adjoint 匹配） | advantage 条件生成 |
| 笔记 | 本文 | [`LWD.md`](./LWD.md) | [`pi0.6*.md`](./pi0.6*.md) |

---

# 18. 参考文献与资源

- *ALOE: Action-Level Off-Policy Evaluation for Vision-Language-Action Model Post-Training*, arXiv:2602.12691.  
- [项目页](https://rooshy-yang.github.io/aloe/) · [arXiv](https://arxiv.org/abs/2602.12691)
